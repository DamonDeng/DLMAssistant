...
import { updateSession, getConfig, getAllSessions } from "../utils/db";
  ...
  useEffect(() => {
    const loadSessions = async () => {
      try {
        ...
        const filteredSessions = loadedSessions.filter(s => !s.deleted);
        setSessions(filteredSessions);
        
        // Set active session to a new empty session if no sessions exist
        if (filteredSessions.length === 0) {
          const tempSession = createEmptySession();
          setActiveSession(tempSession);
        } else {
          setActiveSession(filteredSessions[filteredSessions.length - 1]);
        }
